...
class QuantumCircuitEnv(gym.Env):
    def __init__(self, num_qubits, max_gates, p_meas=0.02, p_gate1=0.01, p_gate2=0.03, T1=50, T2=70):
        ...
        #self._create_noise_model(p_meas, p_gate1, p_gate2, T1, T2))
        self.error_rates = {
            'measurement': p_meas,
    ...
    def _compute_qubit_pair_entanglement(self, qubit1, qubit2):
        ...
        rho = partial_trace(psi, [q for q in range(self.num_qubits) if q not in [qubit1, qubit2]])
        return entropy(rho)
    ...
    def terminate(self, reward):
        ...
        patience = 10  # Number of steps to wait for stabilization
        ...
        entropy = self._compute_entanglement(None)
    ...
    def _calculate_reward(self):
        current_qfi = self._compute_qfi()
        current_depth = self.qc.depth()
        current_entanglement = self._compute_entanglement(None)
        current_gate_count = len(self.qc.data)

        qfi_improvement = (current_qfi - self.initial_qfi) / max(self.initial_qfi, 1e-10)
        depth_reduction = (self.initial_depth - current_depth) / max(self.initial_depth, 1)
        entanglement_improvement = (current_entanglement - self.initial_entanglement) / max(self.initial_entanglement,
                                                                                            1e-10)
        ...
        if current_entanglement >= 0.95:
            reward += 50
        elif current_entanglement >= 0.9:
            reward += 30
        elif current_entanglement >= 0.85:
            reward += 20
        elif current_entanglement >= 0.7:
            reward += 10
        else:
            reward -= 5
    ...
    def _actual_qfi(self):
        ...
        if not params:
            ...
            job = backend.run(circuit, noise_model=self.noise_model)
            results = job.result()
        ...
        for param in params:
            # Create two circuits with slightly different parameter values
            delta = np.pi / 4
            plus_circuit = circuit.copy()
            minus_circuit = circuit.copy()

            # Bind all parameters with random values, except for the current parameter
            param_binds = {p: random.uniform(0, 2 * np.pi) for p in params if p != param}
            ...
            plus_circuit.save_statevector()
            minus_circuit.save_statevector()
            # Get statevectors for both circuits

            backend = Aer.get_backend('aer_simulator',method='statevector',noise_model=self.noise_model)
            plus_job = backend.run(plus_circuit)
            minus_job = backend.run(minus_circuit)

            plus_result = plus_job.result()
            minus_result = minus_job.result()


            plus_statevector = plus_result.get_statevector(plus_circuit)
            minus_statevector = minus_result.get_statevector(minus_circuit)
            #print("plus_statevector",plus_statevector)
            #print("minus_statevector", minus_statevector)
            # Calculate the QFI for this parameter
            d_state = (plus_statevector.data - minus_statevector.data) / delta
            qfi = 4 * (np.vdot(d_state, d_state).real - np.abs(np.vdot(plus_statevector.data, d_state)) ** 2)

            qfi_total += qfi.real

        qfi = qfi_total / len(params) if params else qfi_total
        qfi = max(0,min(1,qfi))
        print("QFI of the circuit",qfi)

        # Normalize QFI by the number of parameters
        return qfi
    def _compute_entanglement(self, circuit):
        ...
        entanglement = 0
        for i in range(self.num_qubits):
            rho_i = partial_trace(psi, [j for j in range(self.num_qubits) if j != i])
            eigenvalues = np.real(np.linalg.eigvals(rho_i.data))
            entanglement -= sum(ev * np.log2(ev) for ev in eigenvalues if ev > 0)
        entanglement_entropy = entanglement / self.num_qubits
        entanglement_entropy = max(0,min(1,entanglement_entropy))
...
def visualize_training_results(rewards, depths, qfis, entanglements, initial_depths,
    ...
    for metric_name, values in metrics.items():
        ...
        plt.grid(True)
        plt.legend()
        plt.savefig(f'25S/{metric_name}_per_Episode.png', dpi=300)
        ...
        plt.legend()
        plt.savefig('25S/Pareto_Front_Depth_Entanglement.png', dpi=300)
        '''
        plt.show()
        ...
        plt.savefig('25S/learning_curve.png', dpi=300)
        plt.close()
        ...
        plt.savefig('25S/pareto_front_3d.png', dpi=300)
        plt.close()
        ...
        plt.ylabel('Episode')
        plt.savefig('25S/optimization_progress_heatmap.png', dpi=300)
        plt.close()
        ...
        plt.legend()
        plt.savefig('25S/error_rates.png', dpi=300)
        plt.close()
        ...
        plt.legend()
        plt.grid(True)
        plt.savefig('25S/Initial_vs_Final_Entanglement.png', dpi=300)
        plt.show()
        plt.close()
        ...
        plt.legend()
        plt.grid(True)
        plt.savefig('25S/Initial_vs_Final_QFI.png', dpi=300)
        plt.show()
        plt.close()
        ...
        plt.grid(True)
        plt.legend()
        plt.savefig('25S/episode_times.png', dpi=300)
        plt.close()
        ...
        plt.grid(True)
        plt.legend()
        plt.savefig('25S/episode_memories.png', dpi=300)
        plt.close()
...
def train_ddqn_agent(num_episodes, num_qubits, max_gates, p_meas=0.02, p_gate1=0.01, p_gate2=0.03, T1=50, T2=70):
    ...
    for e in range(size):
        ...
        for _ in range (max_gates * 10):  # Allow more steps for optimization
            ...
            if((best_depth >= env.qc.depth() or best_gates >= len(env.qc.data)) and best_entanglement <= current_entanglement and current_qfi >= best_qfi) :
                best_circuit = env.qc.copy()
        ...
        '''
        env.qc = best_circuit.copy()

        final_qfi = env._compute_qfi()
        final_entanglement = env._compute_entanglement(best_circuit)
        ...
        final_depth = best_circuit.depth()
        print(
        ...
        np.save("25S/episode_memories.npy", episode_memories)

    agent.save(f"25S/ddqn_model_episode_{e}_25S.weights.h5")
    ...
    plt.savefig('25S/tket_vs_qiskit.png', dpi=300)
    plt.close()
    return agent
...
def test_trained_agent(agent, num_qubits, num_gates):
    ...
    initial_state = env.reset(None)
    #env.qc = create_circuit()
    print("Initial Circuit:")
    print(env.qc)
    initial_circuit_image = circuit_drawer(env.qc, output='mpl')
    initial_circuit_image.savefig("25S/initial_QCirc56.png")
    ...
    for _ in range(num_gates * 200):  # Allow more steps for optimization
        action = agent.act(state)
        next_state, reward, done, _ = env.step(action)
    ...
    final_circuit_image = circuit_drawer(best_circuit,
                                         output='mpl',
                                         fold=10)
    final_circuit_image.savefig("25S/final_QCirc56.png")
    ...
    if(final_qfi1 >= final_qfi and final_entanglement1 >= final_entanglement) :
        ...
        final_circuit_image.savefig("25S/reduced_circ.png")
...
if __name__ == "__main__":
    ...
    trained_agent = train_ddqn_agent(num_episodes=50, num_qubits=25, max_gates=150)
    total_time = time.time() - start_time
    print(f"Total training time: {total_time:.2f} seconds")
    '''''
    env = QuantumCircuitEnv(3, 30)
    state_size = env.observation_space.shape
    action_size = env.action_space.n
    trained_agent = DDQNAgent(state_size, action_size)
    trained_agent.load("25S/ddqn_model_episode_299_25S.weights.h5")
    # Test the trained agent
    test_trained_agent(trained_agent, num_qubits=3, num_gates=30)
    '''''
